...
If you ever need to modify these assets (e.g. change the source code for a built-in shader), you would modify the asset in `bsf/Data/Raw/` (e.g. shader code in `bsf/Data/Raw/Shaders`) and then run the import tool to compile that raw asset into a form usable by the engine. The import tool  basically calls @b3d::Importer::Import same as if you were importing assets manually in your application.

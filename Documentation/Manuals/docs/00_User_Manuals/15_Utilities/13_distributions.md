...
There are different types of distributions:
 - @b3d::TDistribution<float> - Distribution that outputs a single floating point number. Curves are represented using a **TAnimationCurve<float>**, as described in the [animation curves](animCurves) manual.
 - @b3d::TDistribution<Vector2> - Distribution that outputs a 2D vector. Curves are represented using a **TAnimationCurve<Vector2>**, as described in the [animation curves](animCurves) manual.
 - @b3d::TDistribution<Vector3> - Distribution that outputs a 3D vector. Curves are represented using a **TAnimationCurve<Vector3>**, as described in the [animation curves](animCurves) manual.
 - @b3d::ColorDistribution - Distribution that outputs a color. Curves are represented using a **ColorGradient**, as described in the [color gradient](colorGradient) manual.

...
---
Animation curves represent a set of values placed along a timeline that can be smoothly interpolated. They are most often used by the animation system, as well as the particle system (for setting up time-varying properties). The curve is represented with a template @b3d::TAnimationCurve<T> class which can be used with a **float**, **Vector2**, **Vector3** or a **Quaternion** parameter.

Entries in the curve are represented with the @b3d::TKeyframe which can be used with the same parameter types. Each keyframe consists of four values:

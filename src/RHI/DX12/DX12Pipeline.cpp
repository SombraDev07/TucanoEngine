...
std::shared_ptr<RootSignature> DX12Device::createRootSignature(bool allowInputLayout) {
  ...
  // 1: CBV b1
  // 2: CBV b2
  // 3: unbounded SRV table t0 (bindless heap)
  // 4: sampler table s0
  ...
  vdesc.Desc_1_1.NumParameters = 5;
...
std::shared_ptr<RootSignature> DX12Device::createComputeRootSignature() {
  ...
  if (FAILED(hr)) {
    const char* msg = err ? static_cast<const char*>(err->GetBufferPointer()) : "serialize compute root sig failed";
    throw std::runtime_error(msg);
  }
...
std::shared_ptr<PipelineState> DX12Device::createComputePipeline(const ComputePipelineDesc& desc) {
  ...
  throwIfFailed(m_device->CreateComputePipelineState(&pso, IID_PPV_ARGS(&out->pso)), "CreateComputePipelineState");
  return out;
}
